...
export default class Guild {
    ...
    public async getEmojiById(id: Snowflake, stringFormat?: boolean): Promise<string | Emoji | undefined> {
        const emoji = await this.emojis.cache.get(id) as unknown as Emoji

        if (emoji) {
            if (stringFormat) {
                let emojiName = `:${emoji.label}:${emoji.id}>`
                return "<" + (emoji.isAnimated ? "a" + emojiName : emojiName)
            }
            return emoji
        }
    }
